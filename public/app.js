...
const FILTERS = {
  ...
  toDelete: (d) => ["Wrong Number", "Failed Number", "Lead NOT Found", "Bad Lead", "Out of State"].includes(d),
};
const FILTER_LABEL = {
...
function renderSummary(s) {
  ...
  const view = {
    ...
    toDelete: (s.wrongNumber ?? 0) + (s.failedNumber ?? 0) + (s.leadNotFound ?? 0) + (s.badLead ?? 0) + (s.outOfState ?? 0),
  };
