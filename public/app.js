...
const els = {
  ...
  logsBtn: $("logsBtn"),
  closeLogsBtn: $("closeLogsBtn"),
  logsDrawer: $("logsDrawer"),
  logsBody: $("logsBody"),
  tableBody: $("leadTableBody"),
...
function badgeClass(disp) {
  return "badge " + String(disp).replace(/[^A-Za-z]/g, "");
}
...
function renderSummary(s) {
  if (!s) return;
  const map = {
    total: s.total, pending: s.pending, textSent: s.textSent,
    leadNotFound: s.leadNotFound, propertySold: s.propertySold, listed: s.listed,
    optedOut: s.optedOut, needsReview: s.needsReview, errors: s.errors,
  };
  document.querySelectorAll(".stat").forEach((el) => {
    const k = el.dataset.k;
    if (k in map) el.querySelector(".stat-num").textContent = map[k] ?? 0;
  });
}

function renderRows(rows, activeIndex = -1) {
  if (!rows || rows.length === 0) {
    els.tableBody.innerHTML = `<tr class="empty-row"><td colspan="15">Upload a spreadsheet to begin.</td></tr>`;
    return;
  }
  els.tableBody.innerHTML = rows
    .map(
      (r, i) => `
    <tr id="row-${r.rowNumber}" class="${i === activeIndex ? "row-active" : ""}">
      <td>${r.rowNumber}</td>
      ...
      <td>${esc(r.lastContactDate)}</td>
      <td>${esc(r.optOutStatus)}</td>
...
function updateRow(r) {
  ...
  if (!tr) return;
  tr.outerHTML = renderSingle(r);
}
function renderSingle(r) {
  return `
    <tr id="row-${r.rowNumber}">
      <td>${r.rowNumber}</td><td>${esc(r.ownerName)}</td><td>${esc(r.propertyAddress)}</td>
      <td>${esc(r.city)}</td><td>${esc(r.state)}</td><td>${esc(r.zip)}</td>
      <td>${esc(r.reiMatchStatus)}</td><td>${esc(r.lastContactDate)}</td><td>${esc(r.optOutStatus)}</td>
      <td>${esc(r.propertyStatus)}</td><td>${esc(r.eligibilityStatus)}</td>
      <td><span class="${badgeClass(r.disposition)}">${esc(r.disposition)}</span></td>
      <td class="notes">${esc(r.notes)}</td><td>${esc(r.textSentTimestamp)}</td><td class="error">${esc(r.errorLog)}</td>
    </tr>`;
}
...
function setStatus(status) {
  ...
  const canExport = hasJob;
  els.exportXlsxBtn.disabled = !canExport;
...
function setProgress(cursor, total) {
  ...
  const pct = Math.min(100, Math.round((cursor / total) * 100));
  ...
  try {
    ...
    els.logsBody.innerHTML = logs.length
      ? logs
          ...
          .reverse()
          .map(
            (l) => `<div class="log-entry ${l.error ? "err" : ""}">
              <div class="ts">${esc(l.ts)} · row ${esc(l.row)}</div>
              ...
              <div>property: ${esc(l.propertyStatus || "-")} · last contact: ${esc(l.lastContactDate || "-")}</div>
    ...
    els.logsBody.innerHTML = `<p class="muted">${esc(err.message)}</p>`;
  }
};
